...
const UserBox = defineAsyncComponent(
  () => import("@/components//personalCenter/userfocus/userBox.vue")
);
...
const switchTag = async () => {
  ...
  switch (routeQuery.tag) {
    case "0":
      ...
      //请求对应用户的全部关注列表
      //请求对应用户的信息
      const focus = await UserApi.getUserInfo(routeQuery.account);
    ...
    case "1":
      ...
      //请求对应用户的全部粉丝列表
      //请求对应用户的信息
      const fans = await UserApi.getUserInfo(routeQuery.account);
