<template>
    <div class="container">
        <div class="middle">
            ...
            <Suspense :key="messageBoxkey">
                <template #default>
                    <leaveMessage v-for="item in messageList" :key="item.id" :item="item">
                    </leaveMessage>
                </template>
                <template #callback>
                    <div class="window">
...
//请求留言列表
const { data: res } = await useAxios.get('/getmessages')
...
const sendMessage = async () => {
    if (!session) {
        ...
        if (writeMessage.value === '') {
            ...
            const userAccount = JSON.parse(window.atob(session))
            const { data: res } = await useAxios.get('/uploadmessage', {
                params: {
                    account: userAccount.account,
                    content: writeMessage.value
                }
            })
            if (res.status === 0) {
                ElMessage({
