  ...
  <div class="commentMiniBox-container">
    ...
    <div class="headBox">
      <img
        ...
        alt="head"
        @click="goToPersonalCenter(userInfo)"
      />
    </div>
    <!-- 内容 -->
    <div class="bodyBox">
      ...
      <div class="title">
        ...
        >
        <span>&nbsp;&nbsp;回复&nbsp;</span>
        <span class="replyName" @click="goToPersonalCenter(replyInfo)"
          >@{{ replyInfo.nickName }}：
        </span>
        <p>{{ reply.content }}</p>
      </div>
...
//请求对应的用户的信息(这个box属于谁)
const res = await UserApi.getUserInfo(reply.account);
const userInfo = res.data;

//请求回复(@) 的人的信息
const replyUser = await UserApi.getUserInfo(reply.replyTo);
...
const goToPersonalCenter = (info: any) => {
  if (token) {
    ...
    if (tokenInfo.account === info.account) {
