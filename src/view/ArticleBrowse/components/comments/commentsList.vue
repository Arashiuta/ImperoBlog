<template>
  ...
  >
    ...
    <Suspense v-for="item in comments.comment" :key="item.id">
      <template #default>
        <CommentBox :item="item" :articleId="commentsInject.id"> </CommentBox>
      </template>
      <template #fallback>
        <Loading2></Loading2>
      </template>
...
const CommentBox = defineAsyncComponent(
  () => import("@/view/ArticleBrowse/components/comments/commentBox.vue")
);
...
watchEffect(() => {
  if (pinia.articleCommentsAdd) {
    //如果评论更新了就重新请求评论列表
    ifHaveComments.value = true;
    ArticleApi.getIdArticle(commentsInject.id).then((res: any) => {
