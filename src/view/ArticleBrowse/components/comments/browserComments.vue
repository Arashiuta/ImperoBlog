    ...
    <div class="addComment">
      ...
      <div class="commentsend">
        <BtnSend class="btnSend" @click="addComment" v-if="ifAddComment">
          <template v-slot> 点击发布 </template>
        </BtnSend>
        <BtnSend class="btnSend" v-else>
          <template v-slot> 发布中.. </template>
        </BtnSend>
      </div>
    </div>
    <!-- 更新评论列表dom -->
...
import { ref } from "vue";
import commentsList from "@/view/ArticleBrowse/components/comments/commentsList.vue";
...
const addComment = async () => {
  if (!token) {
    ...
    if (commentTextInfo.value === "") {
      ...
      const userAccount = JSON.parse(window.atob(token));
      const res = await ArticleApi.addArticleComment(
        props.articleId,
        userAccount.account,
...
.browserComment-container {
  ...
  .addComment {
    ...
    box-sizing: border-box;
    padding: 1rem;
    border-bottom: 0.2rem solid var(--gray-light-border);
