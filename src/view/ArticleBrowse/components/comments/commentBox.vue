<template>
  <div class="commentBox-container">
    ...
    <div class="contentBox">
          ...
          <el-dialog v-model="dialogVisible" title="Tips" width="30%">
            ...
            <template #footer>
              <span class="dialog-footer">
                ...
                <el-button type="primary" @click="sendDelComment">
                  确定</el-button
                >
              </span>
            </template>
      ...
      <ReplyCommentBox
        v-if="ifReplyComment"
        @cancelReplyComment="cancelReplyComment"
...
import ReplyCommentBox from "@/view/ArticleBrowse/components/comments/replyCommentBox.vue";
import CommentMiniBox from "@/view/ArticleBrowse/components/comments/commentMiniBox.vue";
...
//请求对应用户的头像和昵称信息
const res = await UserApi.getUserInfo(commentInfo.account);
...
const sendDelComment = async () => {
  ...
  const res = await ArticleApi.delArticleComment(
    props.articleId,
    commentInfo.id
  );
  if (res.status === 0) {
...
const goToPersonalCenter = () => {
  if (token) {
    const tokenInfo = JSON.parse(window.atob(token));
    if (tokenInfo.account === props.item.account) {
...
const cancelReplyComment = (val: boolean) => {
  //取消回复评论的组件（子传父false）
...
.commentBox-container {
  ...
  .contentBox {
    ...
    min-width: 10rem;
    padding-bottom: 1rem;
    border-bottom: 0.1rem solid var(--gray-light-border);
