...
onMounted(async () => {
  ...
  if (token) {
    ...
    const res = await ArticleApi.getUserIfParise(tokenInfo.account, props.id);
    if (res.status === 1) {
      //已经点过赞了
  ...
  if (token) {
    const tokenInfo = JSON.parse(window.atob(token));
    const res = await ArticleApi.getUserIfCollection(
      tokenInfo.account,
      props.id
    );
    if (res.status === 1) {
...
const praise = async () => {
  ...
  if (!token) {
    ...
    const tokenInfo = JSON.parse(window.atob(token));
    const res = await ArticleApi.pariseArticle(tokenInfo.account, props.id);
    if (res.status === 0) {
      //点赞成功
      ElMessage({
        message: "点赞成功",
...
const collection = async () => {
  ...
  if (!token) {
    ...
    const tokenInfo = JSON.parse(window.atob(token));
    const res = await ArticleApi.collectionArticle(tokenInfo.account, props.id);
    if (res.status === 0) {
      //收藏成功
      ElMessage({
        message: "收藏成功",
