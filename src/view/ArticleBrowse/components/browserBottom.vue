<template>
  <div class="browserBottom">
    ...
    <div class="others">
      ...
      <div>
        <span>作品标签：</span>
        <span class="labelTag" v-for="tag in articleInfo.tag" :key="tag">{{
          tag
        }}</span>
      </div>
    </div>
...
watchEffect(async () => {
  if (pinia.pariseOrCollection) {
    const res = await ArticleApi.getIdArticle(props.articleInfo.id);
    //更新点赞的数据
...
.browserBottom {
  ...
  .pariseInfo {
    ...
    .pariseNum {
      ...
      align-items: center;
      font-size: 3rem;
      border: 0.3rem solid var(--border-line);
    }
  }
