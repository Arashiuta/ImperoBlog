...
onMounted(() => {
  ...
  document.addEventListener("mousemove", () => {
    if (mouse.x < 960 && mouse.y < 540) {
      img.style.transform = `rotateX(-${mouse.y / 100}deg) rotateY(${
        (mouse.x - 960) / 100
      }deg)`;
...
const login = async () => {
  ...
  const res = await UserApi.userLogin(userLogInfo);
  if (res.status === 2) {
    alert("用户名不存在");
    ...
    localStorage.setItem(
      "userAccount",
      window.btoa(JSON.stringify(accountInfo))
    );
...
const register = async () => {
  ...
  if (registerform.password.length < 6 || registerform.username.length < 6) {
    ...
    if (registerform.password === registerform.againPassword) {
      const res = await UserApi.userRegiser(registerform);
      if (res.status === 0) {
        alert("注册成功!");
        name.value = false;
...
const updateNickName = async () => {
  if (nickname.value === "") {
    ...
    const res = await UserApi.updataNickName(registerInfo.username, uploadNick);
    if (res.status === 0) {
      alert("昵称更新成功");
      registerInfo.password = "";
      registerInfo.username = "";
