<template>
  <div class="userInfo">
    <div class="userhead">
      ...
      <div class="focusUser">
        <div class="focused" v-if="ifFocus" @click="focusDialogVisible = true">
          已关注
        </div>
        ...
        <el-dialog v-model="focusDialogVisible" title="Tips" width="30%">
          ...
          <template #footer>
            <span class="dialog-footer">
              ...
              <el-button type="primary" @click="delFocus"> 确定 </el-button>
            </span>
          </template>
        </el-dialog>
      </div>
...
//请求用户信息
const res = await UserApi.getUserInfo(userPropsAccount);
...
if (logToken) {
  ...
  //请求登录的账号的信息
  UserApi.getUserInfo(logTokenInfo.account).then((res:any) => {
    //请求回执
    const info = res.data.data;
    if (info.focus.indexOf(userInfo.account) !== -1) {
      //已经关注了
      ifFocus.value = true;
    } else {
...
.userInfo {
  ...
  .userhead {
    ...
    .focusUser {
      ...
      .focusClick {
        ...
        box-shadow: 0 0 0.5rem 0.1rem rgb(202, 202, 202);
  ...
  .moreInfo {
    ...
    .title {
      font-size: 2.5rem;
      border-bottom: 0.1rem solid var(--gray-light-sahdow);
      padding: 1rem 0;
    }
