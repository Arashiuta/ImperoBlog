          ...
          <div class="ifHave">
            ...
            <div class="pageInfo">
              ...
              <el-pagination
                ...
                @update:current-page="updataCollectionPage"
              />
            </div>
          </div>
        </el-tab-pane>
          ...
          <div class="ifHave">
            <div class="boxex">
              <Suspense v-for="item in showPush.showArticle" :key="item.id">
                <template #default>
                  <ArticleBox :info="item" class="comBoxex"> </ArticleBox>
                </template>
                <template #fallback>
                  <Loading></Loading>
                </template>
            ...
            <div class="pageInfo">
              ...
              <el-pagination
                background
                layout="prev, pager, next"
                :page-count="push.page.length"
                :current-page="push.nowPage"
                @update:current-page="updataPushPage"
              />
            </div>
          </div>
        </el-tab-pane>
...
import OthersUserInfoCom from "@/view/OthersPersonalCenter/components/othersUserInfo.vue";
...
//请求用户信息
const res = await UserApi.getUserInfo(watchAccount);
...
collectionList.map(async (item: string) => {
  const res = await ArticleApi.getIdArticle(item);
  collectionArray.push(res.data);
});
collectionArray = collectionArray.reverse();

//发布的文章
const list = await UserApi.getPushArticleNum(userInfo.account);
...
let showArticles = ref(
  collectionArray.slice(6 * (pages.nowPage - 1), 6 * (pages.nowPage - 1) + 6)
);
watchEffect(() => {
  //先清空一下数组，再计算页数
  pages.page = [];
  ...
  showArticles.value = collectionArray.slice(
    6 * (pages.nowPage - 1),
    6 * (pages.nowPage - 1) + 6
  ); //页码更新后更新显示的文章内容
});
...
const showPush = reactive({
  showArticle: pushArticleNum.slice(
    6 * (push.nowPage - 1),
    6 * (push.nowPage - 1) + 6
  ),
});
...
watchEffect(() => {
  ...
  showPush.showArticle = pushArticleNum.slice(
    6 * (push.nowPage - 1),
    6 * (push.nowPage - 1) + 6
  ); //页码更新后更新显示的文章内容
});
