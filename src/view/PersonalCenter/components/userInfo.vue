    ...
    <div class="userhead">
      ...
      <Teleport to="body">
        <el-dialog v-model="ifUploadHead" title="更换头像">
          <uploadHeadImg
            :ifsendHead="ifsendHead"
            @uploadHeadSuccess="uploadHeadSuccess"
          ></uploadHeadImg>
          <template #footer>
    ...
    <div class="moreInfo">
      ...
      <div class="titleBottom">
        ...
        <div class="titleBottomDiv">
          ...
          <el-input
            v-model="userInfo.personalWeb"
            placeholder="要加上http://或者https://前缀"
            v-if="ifChangePersonalDate"
            @blur="webIptBlur(userInfo.personalWeb)"
          />
          <a
            v-else
            target="_blank"
            :href="userInfo.personalWeb ? userInfo.personalWeb : 'javascript;'"
            >{{ userInfo.personalWeb ? userInfo.personalWeb : "" }}</a
          >
        </div>
        <div class="titleBottomDiv personalIntroduce">
          ...
          <el-input
            ...
            :maxlength="100"
          />
          <p v-else class="personalIptSpan">
            {{
              userInfo.introduce
                ? userInfo.introduce
                : "这个人很懒，什么也没有留下..."
        ...
        <div class="titleBottomBtn">
          <!-- 修改密码 -->
          <div class="btn" @click="changePassword" v-if="!ifChangePersonalDate">
            修改密码
          </div>
          <Teleport to="body">
            <el-dialog v-model="TableVisibleChangePassword" title="修改密码">
              <div class="passwordBox">
                ...
                <el-input
                  v-model.trim="passwordChange.oldPassword"
                  name="oldPassword"
                  maxlength="16"
                  placeholder="输入旧密码"
                  show-word-limit
              ...
              <div class="passwordBox">
                ...
                <el-input
                  v-model.trim="passwordChange.newPassword"
                  maxlength="16"
                  placeholder="输入新密码"
                  show-word-limit
                  type="text"
                ...
                />
                <el-input
                  v-model.trim="passwordChange.newPasswordNext"
                  maxlength="16"
                  placeholder="再次输入新密码"
                  show-word-limit
                  type="text"
              ...
              <template #footer>
                <span class="dialog-footer">
                  <el-button @click="TableVisibleChangePassword = false"
                    >取消</el-button
                  >
                  <el-button type="primary" @click="sendChangePassword"
                    >提交</el-button
                  >
                </span>
              </template>
          ...
          <Teleport to="body">
            <el-dialog v-model="TableVisibleOutlog" title="退出">
              ...
              <template #footer>
                <span class="dialog-footer">
                  <el-button @click="TableVisibleOutlog = false"
                    >取消</el-button
                  >
                  <el-button type="primary" @click="exitAccount"
                    >确定</el-button
                  >
                </span>
              </template>
...
const uploadHeadImg = defineAsyncComponent(
  () => import("@/view/PersonalCenter/components/uploadHead.vue")
);
...
const res = await UserApi.getUserInfo(tokenInfo.account);
const userInfo = reactive(res.data);

const ifChangePersonalDate = ref(false); //是否更改个人资料
const changePersonalDate = () =>
  (ifChangePersonalDate.value = !ifChangePersonalDate.value);
//修改个人资料
const changePersonalIntroduce = async () => {
  ...
  const res = await UserApi.updataUserInfo(changeIntroduceJson);
...
const sendChangePassword = async () => {
  if (
    ...
    passwordChange.newPasswordNext === "" ||
...
const uploadHeadSuccess = (val: boolean) => {
  ...
  // 再发送一次请求，请求新的头像
  UserApi.getUserInfo(tokenInfo.account).then((res: any) => {
    ...
    .titleBottom {
      ...
      .titleBottomBtn {
        ...
        .btn {
          ...
          border: 0.15rem solid var(--border-line);
          transition: all 0.1s;
        ...
        .btnLogout {
          border: 0.15rem solid rgb(219, 37, 37);

          &:hover {
            cursor: pointer;
            box-shadow: 0 0 0.5rem 0.1rem rgb(219, 37, 37);
          }
        }

        .btnOver {
          border: 0.15rem solid rgb(37, 219, 52);

          &:hover {
            cursor: pointer;
            box-shadow: 0 0 0.5rem 0.1rem rgb(37, 219, 52);
          }
        }
      }
    }
