...
import userInfoCom from "@/view/PersonalCenter/components/userInfo.vue";
...
collectionList.map(async (item: string) => {
  ...
  if (res.status === 0) {
    //如果status为1就是这个文章已经不存在了（被删除），就不添加到数组里面了，不然会报错
...
const showArticles = reactive({
  showArticle: collectionArray.slice(
    6 * (pages.nowPage - 1),
    6 * (pages.nowPage - 1) + 6
  ),
});
watchEffect(() => {
  ...
  showArticles.showArticle = collectionArray.slice(
    6 * (pages.nowPage - 1),
    6 * (pages.nowPage - 1) + 6
  ); //页码更新后更新显示的文章内容
});
